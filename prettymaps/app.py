...
with cols[1]:
    ...
    if button:
        ...
        with st.spinner("Generating map..."):
            ...
            # Paste the map into the center
            final_img.paste(map_img, (side_margin, top_margin))
            
            draw = ImageDraw.Draw(final_img)
            
            # Draw Title exactly aligned with the left edge of the map (side_margin)
            display_title = query.split(',')[0].upper() if isinstance(query, str) else "MAP"
            
            # Fallback to default font if we can't load a custom one
            ...
            # Draw Footer exactly aligned with the left edge of the map (side_margin)
