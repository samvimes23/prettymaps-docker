...
with cols[1]:
    ...
    if button:
        ...
        with st.spinner("Generating map..."):
            ...
            # Add custom title and copyright in the margins
            display_title = query.split(',')[0].upper() if isinstance(query, str) else "MAP"
            fig.text(0.5, 1.0, display_title, ha='center', va='bottom', fontsize=32, fontweight='bold', fontfamily='serif', color='#2F3737')
            fig.text(0.5, 0.0, "Map data © OpenStreetMap contributors | Generated by Prettymaps", ha='center', va='top', fontsize=12, fontfamily='sans-serif', color='#475657')
            
            buf = io.BytesIO()
            plt.savefig(buf, format="png", bbox_inches="tight", pad_inches=0.15, dpi=150)
            buf.seek(0)
